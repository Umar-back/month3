...
async def start_questions(message: types.Message, state:FSMContext):
    await message.answer('Напишите отмена для остановки опроса')
...
async def process_age(message: types.Message, state: FSMContext):
    ...
    if not age.isdigit():
        await message.answer('Нужно ввести цифры')
    elif int(age)<12 or int(age)>99:
        ...
        kb = types.ReplyKeyboardMarkup(
            keyboard=[
                [types.KeyboardButton(text='Мужской'), types.
...
async def process_gender(message: types.Message, state: FSMContext):
    ...
    if gender in ('Мужской', 'Женский'):
        ...
        await message.answer('В какой стране в живете?', reply_markup=kb)


    else:
        await message.answer('нужно ввести муж или жен!')
